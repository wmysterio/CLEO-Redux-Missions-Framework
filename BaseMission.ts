...
export abstract class BaseMission extends BaseScript {
    ...
    /** TESTED !!!
     * Loads a texture dictionary relative to the project directory for sprite rendering.
     * @param name - The TXD file name without extension (e.g., "mytxd").
     * @param spriteNamesToLoad - Optional names of sprites to prepare for rendering from the TXD.
     
    public loadProjectTxdDictionary(name: string, ...spriteNamesToLoad: string[]): void {
        let relativePathToTxd = `${Core.GetProjectAt(Core.RunnedProjectIndex).relativeDirectoryPath}\\${name}.txd`;
        let pathLength = relativePathToTxd.length;
        //log(relativePathToTxd);
        if (!Fs.DoesFileExist(relativePathToTxd))
            throw new Error(`File '${relativePathToTxd}' not found!`);
        if (pathLength > 258)
            throw new Error(`Incorrect path '${relativePathToTxd}' length!`);
        try {
            TxdStore.RemoveScriptTextureDictionary();
        } catch { }
        let pScriptSlotName = Core.CopyStringToMemory("script");
        let slot = -1;
        try {
            slot = TxdStore.FindTxdSlot(pScriptSlotName);
        } catch { }
        Memory.Free(pScriptSlotName);
        let pRelativePathToTxd = Core.CopyStringToMemory(relativePathToTxd);
        let state = 0;
        try {
            state = TxdStore.LoadTxd(slot, pRelativePathToTxd);
        } catch { }
        Memory.Free(pRelativePathToTxd);
        if (state === 0)
            throw new Error(`Cannot read file '${relativePathToTxd}'!`);
        let spriteNamesToLoadLength = spriteNamesToLoad.length;
        for (let i = 0; i < spriteNamesToLoadLength; ++i)
            Txd.LoadSprite(i + 1, spriteNamesToLoad[i]);
    }*/
