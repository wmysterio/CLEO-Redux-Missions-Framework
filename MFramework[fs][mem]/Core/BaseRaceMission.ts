...
import { StreetRacerInfo } from "./StreetRacerInfo";
...
export abstract class BaseRaceMission extends BaseMission {
    ...
    private baseRaceMissionStreetRacerInfos: StreetRacerInfo[];
    ...
    protected onInitEvent(): void {
        ...
        this.baseRaceMissionStreetRacerInfos = new Array<StreetRacerInfo>();
    ...
    protected addStreetRacer(carModel: int, x: float, y: float, z: float, heading: float, charModelId: int = -1, carSetup: (car: Car) => void = undefined): void {
        if (this.baseRaceMissionStage === 0) {
            ...
            let addedRacerInfo = new StreetRacerInfo(carModel, x, y, z, heading, 8 > charModelId ? -1 : charModelId, carSetup);
            this.baseRaceMissionStreetRacerInfos.push(addedRacerInfo);
        }
    }
    ...
    protected addStreetRacerAsPlayer(carModel: int, x: float, y: float, z: float, heading: float, carSetup: (car: Car) => void = undefined): void {
        if (this.baseRaceMissionStage === 0) {
            if (carSetup === undefined)
                carSetup = BaseRaceMission.baseRaceMissionEmptyCarSetup;
            let addedRacerInfo = new StreetRacerInfo(carModel, x, y, z, heading, 0, carSetup);
            this.baseRaceMissionStreetRacerInfos.push(addedRacerInfo);
        }
    }
    ...
    private baseRaceMissionProcessAfterBegin(): void {
        ...
        let numAddedRacerInfo = this.baseRaceMissionStreetRacerInfos.length;
        let usedModels = new Array<int>();
        for (let i = 0; i < numAddedRacerInfo; ++i) {
            usedModels.push(this.baseRaceMissionStreetRacerInfos[i].carModel);
            let charModelId = this.baseRaceMissionStreetRacerInfos[i].charModelId;
        ...
        for (let i = 0; i < numAddedRacerInfo; ++i) {
            let racerInfo = this.baseRaceMissionStreetRacerInfos[i];
